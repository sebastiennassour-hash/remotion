...
| `render.ts` | Rend tous les posts en MP4 + PNG et la planche contact dans `out/`. |
| `captions.ts` | Génère `out/LEGENDES.md` avec le texte à l’écran et la légende de chaque post. |
...
# Options
bun render.ts --stills-only
bun render.ts --only=VA-05-Reels
